...
# Play around with recognizing one of our typical cross shaped alignment targets

import cv2
import numpy


class CrossTarget(object):
    '''Find a cross target'''

    def __init__(self):
        # Color threshold values, in HSV space
        self.low_limit_hsv = numpy.array((40, 30, 50), dtype=numpy.uint8)
        self.high_limit_hsv = numpy.array((80, 255, 255), dtype=numpy.uint8)
    ...
    def process_image(self, camera_frame):
        ...
        contour_list = []
        for c in contours:
            center, widths = CrossTarget.contour_center_width(c)
        ...
        cv2.drawContours(camera_frame, [biggest_contour], -1, (0, 0, 255), 1)
        ...
        # poly_fit = CrossTarget.quad_fit(contour_list[0]['contour'], 0.01)
        # cv2.drawContours(camera_frame, [poly_fit], -1, (100, 255, 255), 1)

        hull = cv2.convexHull(biggest_contour)
        hull_fit = CrossTarget.quad_fit(hull, 0.01)
        #cv2.drawContours(camera_frame, [hull], -1, (0, 255, 0), 1)
        cv2.drawContours(camera_frame, [hull_fit], -1, (255, 0, 0), 1)
